...
gist costs $0.27\times$---it dominates; on the official NVIDIA RULER suite (Llama,
16K) the flagship and the 2-bit arm are indistinguishable while eviction at
$0.1\times$ collapses. Low-rank compresses a feature axis and
...
2-bit floor, with full single/multi-key retrieval and $0.83$--$1.00$ multi-value
on Llama and Mistral (at a fluency cost; Qwen diverges). We are explicit that today's storage ratio is not deployed VRAM:
...
quantization; what it does not share is the $1/T$ term---its stored ratio falls
from $0.151\times$ to $0.133\times$ between 16K and 64K on Llama, with full
four-task retrieval at 64K, while any fixed-bit quantizer stays flat.
\end{abstract}
...
subspace, so the online-DLRA axis remains, to our knowledge, unoccupied. This version adds the two baselines the first draft lacked---a
KIVI-faithful 2-bit quantizer at matched bytes (\S\ref{sec:quantbaseline}) and an
...
loses, and pays for it in fluency; 4-bit quantization at about twice the bytes
concedes nothing; and on the official RULER prompts (\S\ref{sec:realistic}) the
multi-value edge does not reproduce and the two arms are indistinguishable. What
...
$1.00$ for 4-bit KIVI and Palu, $p{=}0.008$). Its 14 misses sit at needle depths
$0.15$--$0.95$, so this is not a warm-up-window or recency effect but scattered
misses the quantizers do not make. We therefore state the matched-bytes
...
(Tables~\ref{tab:xmodel},~\ref{tab:xmodel32}), and keeps falling. We measured
the next point (Figure~\ref{fig:one_over_t}): at 64K on Llama the flagship's
honest stored ratio is $0.133\times$ ($0.151\to0.140\to0.133$ from 16K to 64K)
while the 2-bit quantizer holds $0.156$--$0.158\times$; and at that point the
flagship retrieves $1.00$ on all four tasks ($n{=}8$) where the 2-bit arm scores
$1.00/0.58/0.50/1.00$ and eviction at $0.1\times$ scores $1.00/0.67/0.83/1.00$
($n{=}12$), at perplexity $8.59$ against $8.15$ (2-bit and eviction), $7.27$
(4-bit) and $7.26$ (full KV). This is the one frontier statement no scalar
quantizer can match at long enough context: a $b$-bit quantizer is floored at
$b/16$, while a bounded-rank tracker is not floored at all. It is also why the
persistence win (\S\ref{sec:memory} below) grows with context.
...
three-family, 32K official grid and a flagship LongBench sweep remain open.
...
persistence at \texttt{5e8b275}, official anchor at \texttt{c331ebd}, the 64K
point at \texttt{d15769d}) follow the same SHA-pinned, per-trial protocol and are
paired with the Week-18 flagship records needle-for-needle; the 64K flagship
cells are $n{=}8$ (its comparators $n{=}12$). Two
