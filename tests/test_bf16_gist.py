...
* **The basis stops being orthonormal** -- ``‖UᵀU - I‖_F`` ~ 2.8e-3 at the tiny model's
  ``n=32, r=8`` (4.6e-3 at ``n=1024, r=64``, via ``kvdlra.eval.recon.bf16_gist_drift``),
  above ``orth_fix_tol`` (1e-3) and far below ``orth_abort_tol`` (1e-1), so the guard
  repairs it every absorb but the first.
* **The coordinates are re-rounded at every carry** and nothing repairs those, so their
  error compounds with the absorb count -- the risk ``prereg/bf16_gist.md`` names. Both
  points in full, with the drift curve, in the arm's ``doc:``; 1000 absorbs is not a
  90-second suite, so the light test below pins only direction and shape at a tiny size.
...
def _cache(model: LlamaForCausalLM, **kw: Any) -> BugStreamingCache:
    ...
    return tiny_cache(model, **{**base, **kw})
...
# Module-scoped and mutable: test_the_guard_repairs_every_absorb_under_bf16_storage below
# drains both caches' diag rows via drain_diag(). A test added after it that needs diag
# rows would silently get an empty list -- build a local cache (``_run(...)``) instead.
...
def test_bf16_storage_halves_the_gist_bits_and_keeps_the_reconstruction_close(
    ...
    # The bf16 drift budget on this stream: measured 1.4e-2 over these 44 absorbs, and
    # growing with the absorb count (the curve is in the arm's `doc:`).
    assert rel < 5e-2, rel
...
# ------------------------------------------------------ the drift generator (L5.1 fix1)


def test_bf16_gist_drift_shape_and_direction() -> None:
    """``kvdlra.eval.recon.bf16_gist_drift`` is the committed generator behind the arm's
    ``doc:`` and the task report's drift curve (40/200/1000 absorbs is a script run, not
    a test -- see the report). This pins its return shape and direction at a tiny size:
    the bf16-stored gist measurably drifts from the fp32 one, and the bf16-rounded basis
    sits further from orthonormal than the fp32 one does."""
    result = bf16_gist_drift(n=64, rank=8, block=8, absorbs=4, seed=0)
    assert result.keys() == {"rel_error", "orth_error_bf16", "orth_error_fp32"}
    assert result["rel_error"] > 0.0
    assert result["orth_error_bf16"] > result["orth_error_fp32"]
