...
7. **`rope_basis: post`** is one identity in the shared rotation helper `_mat_rope_with`, reached from the three ingest un-rotations and the reconstruct re-rotation (the four call sites below became one early return in the final fix wave) and ships with the `isvd_postrope_r128` arm, a single-arm pod paired against the Gate-1 Stage-1 Llama pod by `prompt_sha256` / `window_idx` (the pre-flight re-run's precedent, D-011 addendum 10), and its own prereg; the pod is not launched in this lane.
...
### Task 10: `pre_run` — the pod runs a declared command before `pod.py run`, and its exit code is harvested and checked (the mechanism Amendment 1 A1.2 promised)

**Why.** `prereg/kernel_smoke.md` Amendment 1 A1.2 says the Triton kernel's own tests (`pytest -m gpu`, `tests/test_kernel_triton.py` and the gpu params of `tests/test_kernel_reference.py`) "are run on the pod before `pod.py run`, from the same clone; their output is pasted into the launch entry". Nothing performed that step: `scripts/pod/boot.sh` clones at `$SHA`, pip-installs the evaluation stack (NOT the `[dev]` extras — no pytest on the pod) and hands off to `pod.py run` under `timeout`. A pod launched then would have run the kernel_check axis and the latency cells without ever executing the kernel's correctness gate, and A1.2 would have been false. Controller ruling R-L4-27: a generic, minimal `pre_run` hook, recorded and checked like every other pod output; the launch waits for it.

**Files:**
- Modify: `scripts/pod/boot.sh` — after `===ENV_END===` and BEFORE the `timeout` hand-off: when `$PRE_RUN` is non-empty, `pip install -q pytest 2>&1 | tail -1`, then `echo "===PRE_RUN_BEGIN_${POD}==="`, `bash -o pipefail -c "$PRE_RUN"` (its stdout/stderr go to the pod log like everything else), `rc=$?`, `echo "===PRE_RUN_END_${POD}_rc=${rc}==="`. The pod is NOT aborted on rc ≠ 0 (the check axis and the latency cells still run and are recorded; `check` is what refuses) and the hook is not under the `timeout` (that bar is the measurement's; the hook has its own — pytest on the gpu items is minutes). The comment block at the top gains the hook and its markers.
- Modify: `src/kvdlra/eval/config.py` — `PodCfg` gains `pre_run: str | None = None` (an optional shell command string; part of the YAML and therefore of `config_hash`); `load_pod` passes it through and refuses a blank one. `config_hash` drops the key when it is None, so a pod that declares no hook hashes exactly as before — otherwise a field with a default would have moved every committed manifest's hash and failed `make check`.
- Modify: `scripts/pod.py` — `launch_command`: a declared `pre_run` appends `-e PRE_RUN=<shlex.quote(command)>` to the one `--env` string (the dry run prints it); `harvest`: `===PRE_RUN_END_<pod>_rc=(\d+)===` → `manifest["pre_run_rc"]` (int, None when absent), and every `[pre_run] `-prefixed row, prefix stripped, → `results/<pod>/pre_run.txt` (only when at least one exists; the non-shrink guard covers it, counting lines rather than records); `check`: `_pre_run_fails`, beside the other `_*_fails` and gated like them (skipped on a dry-run manifest) — `CHECK FAIL pre_run: rc=<n>` on a non-zero code, `CHECK FAIL pre_run: not recorded` on None, nothing at all for a pod that declares no hook.
- Modify: `scripts/pod/watchdog.sh` — `ROWS` keeps `^\[pre_run\]` and `^===PRE_RUN` (the markers and the rows must reach the harvested log).
- Modify: `configs/pods/kernel_smoke.yaml` — `pre_run: "python -m pytest -m gpu -q -rA -p no:cacheprovider tests/test_kernel_triton.py tests/test_kernel_reference.py 2>&1 | sed 's/^/[pre_run] /'"`. With `-o pipefail` in boot.sh the recorded code is pytest's; `-rA` prints the per-item summary, so the pasted evidence names each gpu item and the max/rms values the tests print into their assert messages. No `PYTHONPATH=src` prefix: `pyproject.toml`'s `[tool.pytest.ini_options] pythonpath = ["src", "scripts", "."]` puts the package on the path from the clone root with no install. The `doc:` names the hook.
- Modify: `tests/test_pod_manifest.py` — (a) the launch command for `kernel_smoke` carries `-e PRE_RUN=` once, shell-quoted, and round-trips through `shlex`; `gate1_v2_stage1_llama` carries none; (b) `harvest` of a fake log with the two markers and three `[pre_run] …` rows writes `pre_run.txt` (prefix stripped) and `pre_run_rc == 0`, and a log without the markers writes neither; (c) `check` passes the pre_run gate on that harvest, fails `CHECK FAIL pre_run: rc=1` when the code is 1 and `CHECK FAIL pre_run: not recorded` when the markers are absent; (d) the watchdog-rows test gains the two markers and a `[pre_run] ` row; plus a declared `pre_run` moves `config_hash`, and a blank one is refused at load.
- Modify: `tests/test_kernel_smoke_pod.py` — the fake log gains the pre_run block, so the end-to-end `check` still passes and the PRECONDITION/GATE strings still render.
- Modify: `prereg/kernel_smoke.md` — APPEND ONLY: `### Amendment 1 — correction (2026-09-21 06:07 EDT, before the launch commit)`, narrowing A1.2's sentence to the mechanism (the command verbatim, where boot.sh runs it, `pre_run.txt` + `manifest.pre_run_rc`, what `check` refuses, the launch entry citing the file instead of pasting output), stating that no reading, threshold or bar moves and that `config_hash` moves because the YAML gained a key (no manifest existed).
- Modify: `docs/plan/plans/2026-09-20-L4-kernel.md` — this section, in the same commit.

**Interfaces.** Consumes: `PodCfg`, `load_pod`, `config_hash`, `launch_command`, `harvest`'s `parsed`/manifest writing and non-shrink guard, `check`'s `_*_fails` pattern, the watchdog `ROWS`. Produces: `PodCfg.pre_run`, `manifest["pre_run_rc"]`, `results/<pod>/pre_run.txt`, the markers `===PRE_RUN_BEGIN_<pod>===` / `===PRE_RUN_END_<pod>_rc=<n>===`, the `[pre_run] ` row prefix, `scripts/pod.py::_pre_run_fails`.

**Steps.** TDD: the tests of (a)–(d) first, run to failure → config → pod.py launch/harvest/check → boot.sh → watchdog → the YAML → the prereg correction → this section → focused tests with `--durations=5` → `make test` + ruff + mypy + `make tables` diff-clean + the forbidden-word grep → ONE commit by explicit path.

```bash
git add scripts/pod/boot.sh scripts/pod/watchdog.sh scripts/pod.py src/kvdlra/eval/config.py configs/pods/kernel_smoke.yaml tests/test_pod_manifest.py tests/test_kernel_smoke_pod.py prereg/kernel_smoke.md docs/plan/plans/2026-09-20-L4-kernel.md && git commit -o -m "L4.10: pre_run — the pod runs the YAML's declared command before pod.py run (boot.sh, markers, [pre_run] rows), harvested into pre_run.txt + manifest.pre_run_rc, refused by check on rc != 0; kernel_smoke declares its gpu tests (pod kernel_smoke, prereg/kernel_smoke.md Amendment 1 correction)" -- scripts/pod/boot.sh scripts/pod/watchdog.sh scripts/pod.py src/kvdlra/eval/config.py configs/pods/kernel_smoke.yaml tests/test_pod_manifest.py tests/test_kernel_smoke_pod.py prereg/kernel_smoke.md docs/plan/plans/2026-09-20-L4-kernel.md
```

**Fix round 1 (R-L4-29, R-L4-30).** The command travels base64 as `-e PRE_RUN_B64=`, decoded by `boot.sh` into `$PRE_RUN` before the hook and echoed as its first `[pre_run] $ …` row: vast.ai's own `--env` parser (`vastai/utils.py`, read and run on the installed CLI) splits the string on spaces outside quotes and toggles the quote state on every `'`, so the quoted hand-off reached the pod cut at `python -m pytest --version 2>&1 | sed 's/^/[pre_run]` while the base64 token survives byte-intact. `check` gains a third refusal: `pre_run.txt` must carry a pytest counts row with an `N passed` count (`CHECK FAIL pre_run: no passed row`) and that row must name no failure or error (`CHECK FAIL pre_run: failures in the summary`), since `conftest` skips every `gpu` item on a card without CUDA and pytest exits 0 on an all-skipped run — plus the YAML records `pytest --version` ahead of the gate, drops the `-q` that `pyproject.toml`'s `addopts` already supplies (a second one puts pytest at verbosity -2, where it prints no counts row for the new gate to read — measured, and pinned by `test_the_pre_run_gate_leaves_pytest_one_quiet_flag`), and `PRE_RUN_ROW_RE` becomes CRLF-safe.
...
**Name consistency.** `decode_attention` / `kernel_operand_dtype` / `rope_basis` (Tasks 4, 6, 7, 8, 9); `KERNEL_ATTN = "kvdlra_factored"` (Tasks 4, 8); `FactoredMiddle`, `factored_attention(..., backend=)`, `rope_cos_sin` (Tasks 3, 4, 5); `kernel_middles()`, `_factored_middle()`, `_mid_coords()`, `_decode_peek(dense_only=)`, `kernel_compare` (Tasks 4, 6); `_rows`, `_ctor_kwargs`, `drain_diag()` (Tasks 1, 4); `KernelCheckRecord`, `KERNEL_CHECK_RE`, `parse_kernel_check_lines`, `check_prompt`, `failed_row`, `format_line`, `is_kernel_arm`, `_kernel_check_fails`, `TaskKernelCheckCfg` (Tasks 6, 7, 8); `kv_resident_gb`, `latency_table`, `week3_gate`, `precondition_line`, `analytic_stored_gib`, `latency_roles` (Tasks 7, 8 — `latency_roles` became `kvdlra.eval.config.role_of` in the final fix wave); the marker `gpu` (Tasks 2, 5); the arm stems `isvd_r64_h256_seed_kernel`, `isvd_postrope_r128`, the pods `kernel_smoke`, `postrope_r128`, the task `kernel_check_16` (Tasks 4, 6, 8, 9). The per-task "Expected" counts were written against 603 passed at the branch point and 7 gpu-marked skips from Task 5 on; both were stale before Task 1 landed. The lane's actual figures: 626 at `09b9103` (Task 1), 678 passed / 8 skipped at `99ea29f` (Task 10, the launch commit), **680 passed / 8 skipped** at the end of the final fix wave — 8 gpu-marked skips (Task 5's five plus the three parametrized gpu items of `tests/test_kernel_reference.py`), and the 1B-dump item skips too on a tree without the dumps (679 in CI).


## Final fix wave (2026-09-21, after the whole-branch review and the ponytail review)

One dispatch, four commits, every gate green at each. The pod (`kernel_smoke`, instance
51903816 from `a691d44`) ran throughout: `results/`, `configs/pods/kernel_smoke.yaml` and the
existing text of `prereg/kernel_smoke.md` were not touched.

- **(A) `32b9a0a`** — the review's two Important findings plus its one optional test.
  `kvdlra.kernel.attention` hands the kernel the row's own `FactoredMiddle` at batch 1 instead
  of `FactoredMiddle.cat([m])` (a cat of one row copies ≈ 17 MB per layer per step at 32K, r64;
  numerically identical), pinned by an object-identity assertion in `tests/test_kernel_path.py`.
  Backend attestation: `kvdlra.kernel.select_backend` is the one place `"auto"` resolves,
  recorded once per cache (`BugStreamingCache.kernel_backend`, a second, different value
  raises), carried by an optional ` backend=` field on the `[kernel_check …]` and `[latency …]`
  lines, a `backend` key on both records and a `backend` column in the §7 table. And one test
  drives `run_latency`'s REAL print through `records.LATENCY_RE` — the seam the review had to
  check by hand.
- **(B) `b939aea`** — the ponytail cuts of R-L4-32, taken without moving a rendered string or a
  test assertion (`tests/test_golden_cache.py` green throughout): the unreachable batch guard in
  `run_ppl`; the `rope_basis == "post"` identity once in `_mat_rope_with` instead of at four call
  sites; `kernel_operand_dtype` validated through the `GIST_DTYPES` map; the constant-true
  `triton_kernel` probe; `latency_table`'s unused `archive=`; one `role_of(cfg)` in
  `kvdlra.eval.config` for the table, the gate and `_kernel_check_fails`; one corpus-load-and-
  digest helper in the runner; and in tests the layer's own `_factored_middle()` /
  `_decode_peek(dense_only=True)`, the shared `_teacher_forced`, `P50`/`PEAK` and `_mid_to`,
  minus two config-hash sweeps a third already covers. Net −33 lines. The items the reviews
  marked LEAVE were left: `_ensure_mid_cache` via `_mid_coords`, `boot.sh`'s plain-`PRE_RUN`
  fallback, `N_LAYERS`' unused rows, `prompts.py`'s re-export, and `kernel_smoke.yaml`'s `doc:`
  (inside the running pod's `config_hash`).
- **(C) `bcc5f04`** — the prereg notes. `prereg/kernel_smoke.md` gains an APPENDED dated
  "Amendment 1 — correction 3" (nothing above the STATUS line edited; the diff removes no line):
  the preamble's "below it" reads "above it"; A1.2's "met when" carries the renderer's third
  conjunct `errors == 0`; the `backend` field and column exist from `32b9a0a` and the running
  pod's records predate them (harvested as `null`, the launch entry naming the backend from the
  pre_run evidence instead); the one-row `cat` copy is present in the running pod's kernel-arm
  p50 as a conservative ≈ 1–1.5 ms/token handicap; and the absolute 1e-2 precondition bar is read
  on a bf16 output whose quantization scales with |out|, so a token-exact kernel can cross it —
  which reads REFUSED under §4 and is reported as measured, a relative bar being pre-registerable
  only by a further amendment before any re-run. `prereg/postrope_r128.md` (unlaunched) is
  revised in place: the §10 snippet catches the window-pairing raises so every §11 refusal prints.
- **(D) this commit** — the counts above and this note.

Left for the scoped re-review and the clean-clone gate: nothing in `results/`; the DECISIONS
outcome entry is the orchestrator's, written against correction 3.
