...
(Expected~Attention at $0.100\times$) but loses variable-tracking there
(vt $=17$ in our Week-11 table) and its multi-key decays with length
($92\to67\to50$ across $16$K$\to32$K$\to64$K). Scalar 2-bit quantization floors
around $0.125$--$0.19\times$. This $0.043\times$ cell is thus the seed of an
honestly \emph{exclusive} sub-$0.05\times$ band---but establishing exclusivity
against quantization requires the KIVI/KVQuant baseline we defer to v2
(\S\ref{sec:limits}); today we claim only that low-rank reaches a compression
depth the pruning and factorization families do not, and that eviction reaches
depth but not the hard tasks.
% source: results/w11-final-tables.md (bugS-r32-h256 @32K 100/67/100/100 @0.043x); ea-k0.1 profile same file.

\subsection{Moderate compression on realistic text}
\label{sec:realistic}
On a LongBench~\citep{bai2024longbench} document-QA probe (single Qasper document,
$\sim$5.6K tokens), a moderate-compression \texttt{bug-r128} scores F1 $0.221$ at
$0.160\times$, ahead of the sub-$0.25\times$ eviction arms
(Expected~Attention-k0.25 $0.216$ at $0.250\times$; -k0.1 $0.149$ at
$0.100\times$; SnapKV-k0.1 $0.136$ at $0.100\times$) and of \texttt{palu-r0.25}
($0.073$, which collapses on real text); full KV is $0.259$. We present this as an
\emph{illustrative} ordering, not a headline: it is one document (very low power),
uses the pre-seed config, and a full flagship LongBench sweep is future work
(\S\ref{sec:limits}). It does establish that the extreme-compression story is not
purely a synthetic-retrieval artifact.
% source: results/w11-goalA-lb-lines.txt (qasper: bug-r128 0.221@0.160, ea-k0.25 0.216@0.250, ea-k0.1 0.149@0.100, snapkv-k0.1 0.136@0.100, palu-r0.25 0.073, full 0.259).
...
decode-time reconstruction workspace is $\approx0.98\times$ full KV at ctx 2048
and actual decode residency is $\approx1.06\times$ full KV---slightly worse than
no compression. Our only end-to-end latency datum (1B, CPU) has \texttt{bug-r128}
at $224.9$\,ms/token vs full KV $204.5$\,ms/token ($\approx10\%$ slower).
Realizing the stored-state ratio as resident memory requires a fused factored
attention kernel; we state clearly that we have not built one, so on the resident
axis eviction and ThinK ($0.75\times$ resident) currently beat BUG.

Where the stored-state ratio \emph{does} pay off today is persistence and
transfer: a serialized full KV at 32K (Llama-8B, fp16) is $4.29$\,GB, versus
$\approx0.69$\,GB for \texttt{r128-s32} and $\approx0.37$\,GB for
\texttt{r64-h256}---a $\approx6\times$ smaller artifact to store, ship, or reload
for prefix-cache reuse and conversation resumption.
\todo{The $\approx6\times$ is a stored-bytes ratio; the corresponding
\emph{cold-load speedup} is projected, not yet measured. Run w16\_storage on CUDA
and add a measured serialize/reload/H2D number before v2.}
% source: docs/reviews/2026-09-01/review-systems.md (0.98x workspace, 1.06x resident, 224.9 vs 204.5 ms, 4.29GB/0.69GB/0.37GB); task-given fp32-at-rest ratios.
...
\paragraph{No scalar-quantization baseline yet.} The family that most directly
contests the headline band---2-bit KV quantization (KIVI~\citep{liu2024kivi},
KVQuant~\citep{hooper2024kvquant})---is not run in v1. We frame low-rank and
quantization as composing (\S\ref{sec:quant}), and the exclusive-band claim
(\S\ref{sec:subcliff}) is explicitly conditional on that baseline. Running it
(same perplexity$+$RULER harness, three families, plus a BUG$\times$2-bit
composition at $\approx0.04\times$) is the primary v2 experiment.

\paragraph{Benchmark external validity.} Retrieval uses an in-repo RULER-style
generator whose filler is a small fixed sentence pool; selection and confirmation
share the generator. The single-document LongBench probe (\S\ref{sec:realistic})
is a first external anchor but is low-power. A v2 anchor---official RULER subset
and a flagship LongBench sweep, with wikitext/PG19 filler---is planned.
