...
fixed-bit scalar-quantizer floor and below the retrieval any eviction$\times$quantization
composite reaches at matched bytes, with full single/multi-key retrieval and
...
(Table~\ref{tab:composite}). At matched bytes none reaches the cell's retrieval: on
Llama the byte-matched \texttt{ea-k0.25-q2} ($0.039\times$) scores multi-key/multi-value
$0.08/0.08$ against the cell's $1.00/1.00$, and the only composite that matches the
cell's single/multi-key/multi-value (\texttt{ea-k0.25-q4}) needs $0.070\times$---$1.5$--$2\times$
the cell's bytes. On the official RULER anchor (\S\ref{sec:realistic}) every composite
at $\le0.07\times$ collapses to mean $0.11$--$0.33$, at or below plain eviction's $0.20$:
the essay haystack defeats eviction whether or not its survivors are quantized. So the
...
$n{=}12$ (accuracy single/multi-key/multi-value/var-track; honest stored bits).
\texttt{ea-k$k$-q$b$} $=$ Expected~Attention keep-$k$ with $b$-bit survivors. No composite
matches the cell's multi-key/multi-value at $\le$ its bytes; on the official anchor every
composite $\le0.07\times$ collapses to the ``off.'' column, at or below plain eviction.}
\label{tab:composite}
\small
\begin{tabular}{lccc}
\toprule
