...
* **The basis stops being orthonormal.** A bf16-rounded orthonormal basis has
  ``‖UᵀU - I‖_F`` ~ 3.8e-3 at the tiny model's ``n=32, r=8`` (4.6e-3 at ``n=1024,
  r=64``, 6.7e-3 at ``n=512``, via ``kvdlra.eval.recon.bf16_gist_drift``) -- above
  ``orth_fix_tol`` (1e-3) and far below ``orth_abort_tol`` (1e-1). The step propagates
  that error into the basis it returns, so the guard repairs with a thin QR on
  **every** absorb after the first. That is the design, not a defect: the repair is
  what stops U's rounding from compounding.
* **The coordinates are re-rounded at every carry** (``rot @ C`` then back to bf16), and
  ...
  risk ``prereg/bf16_gist.md`` names; the 40/200/1000-absorb drift curve is generated by
  ``kvdlra.eval.recon.bf16_gist_drift`` and quoted in the arm's ``doc:`` and the task
  report, not asserted here (1000 absorbs is not a 90-second suite) -- the light test
  below only pins its direction and shape at a tiny size.
