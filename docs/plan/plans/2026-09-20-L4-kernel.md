...
- The `N passed` numbers in the "Expected" lines assume 603 passed at the lane head (`91d149d`, the count `docs/plan/STATE.md` records for the L3 lane); if your baseline differs, apply the same delta (Task 1: −1 +5; Task 2: +3; Task 3: +6 on this Mac; Task 4: +6; Task 5: +7 skipped; Task 6: +7; Task 7: +6; Task 8: +4; Task 9: +5).
...
Expected: all passed; `test_batched_cache.py` under 2.0 s in the durations list; the golden checksums unchanged.

- [ ] **Step 5: Full suite + lint**

Run: `cd /Users/hari/src/kv-dlra/.claude/worktrees/L4-kernel && .venv/bin/python -m pytest -q && .venv/bin/ruff check src tests && .venv/bin/ruff format --check src tests && .venv/bin/mypy src tests scripts`
Expected: `607 passed` (603 − 1 deleted + 5 new), ruff clean, mypy `Success`.
...
The prereg rule (`prereg/kernel_smoke.md` §4): "The 16 prompts are fixed before the kernel is written, not chosen after seeing its output: they are committed in L4's test module, and the launch entry names their source and selection." The source module holds the constants because the pod runner must import them without the tests package; the test module is the committed, literal record and fails if the source drifts. `prompts.py` imports torch only inside `tiny_prompts()` so `kvdlra.eval.config` (which `scripts/pod.py check` imports in a torch-free subprocess) can import the constants in Task 6.
...
- [ ] **Step 3: Create the package and the prompts module**
...
Expected: `3 passed` (module under 0.3 s); `ok`; the `-m gpu` run reports `no tests ran` (nothing is marked yet) with no `--strict-markers` complaint.

- [ ] **Step 6: Full suite + lint, then commit**

Run: `cd /Users/hari/src/kv-dlra/.claude/worktrees/L4-kernel && .venv/bin/python -m pytest -q && .venv/bin/ruff check src tests && .venv/bin/ruff format --check src tests && .venv/bin/mypy src tests scripts`
Expected: `610 passed`, ruff clean, mypy `Success`.

```bash
cd /Users/hari/src/kv-dlra/.claude/worktrees/L4-kernel && git add src/kvdlra/kernel/__init__.py src/kvdlra/kernel/prompts.py tests/test_kernel_prompts.py tests/conftest.py pyproject.toml && git commit -o -m "L4.2: the 16 correctness prompts fixed before the kernel (pg19-val windows on the pod, seeded draws on CPU; literals pinned) + the gpu pytest marker" -- src/kvdlra/kernel/__init__.py src/kvdlra/kernel/prompts.py tests/test_kernel_prompts.py tests/conftest.py pyproject.toml
```
...
def random_case(
    ...
    c_v = torch.randn(R, T_MID, generator=g) * spectrum
    if contiguous:
    ...
    dense_v = torch.randn(1, H_KV, N_DENSE, D, generator=g).to(dtype)
...
def factored_attention(
    ...
    *,
    ...
    if backend == "auto":
        backend = "triton" if query.is_cuda and _triton_available() else "reference"
    if backend == "triton":
...
Expected: `6 passed` on this Mac (the dump test runs, ~1-2 s; it shows as skipped in CI); the two random bf16 cases report diffs of order 1e-3; total module time under 3.5 s.

- [ ] **Step 5: Full suite + lint, then commit**

Run: `cd /Users/hari/src/kv-dlra/.claude/worktrees/L4-kernel && .venv/bin/python -m pytest -q && .venv/bin/ruff check src tests && .venv/bin/ruff format --check src tests && .venv/bin/mypy src tests scripts`
Expected: `616 passed`, ruff clean, mypy `Success`.
...
function must receive for `q_len > 1`.
"""
...
def test_kernel_step_returns_dense_only_and_the_scope_registers(
    ...
    mid = mids[0]
    assert mid is not None and mid.n_columns == layer._mid_len() > 0
    k_dense, _ = layer._decode_peek(dense_only=True)
    assert int(k_dense.shape[2]) + mid.n_columns == layer.attended_length()
    assert layer.get_mask_sizes(1)[0] == layer.attended_length()  # the mask reports all of it
...
@pytest.mark.parametrize("extra", [{}, TIER], ids=["plain", "exact_tier"])
def test_the_16_prompts_are_token_exact_in_fp32(
    tiny_model: LlamaForCausalLM, extra: dict[str, Any]
) -> None:
    mismatches: list[tuple[int, int]] = []
    for i, prompt in enumerate(tiny_prompts()):
        ids = prompt[None]
    ...
    assert mismatches == [], mismatches  # expected 16/16; the pre-registered bar is >= 14/16
...
def factored_attention_forward(
    ...
    **kwargs: Any,
    ...
    if cache.kernel_compare is not None:
        ...
        ref, _ = sdpa_attention_forward(module, query, k_full, v_full, None, dropout=0.0, scaling=scale)
...
Expected: all passed; `test_kernel_path.py` under 2.5 s total (the two 16-prompt runs ≈ 0.7 s each); the golden checksums unchanged.

- [ ] **Step 7: Full suite + lint, then commit**

Run: `cd /Users/hari/src/kv-dlra/.claude/worktrees/L4-kernel && .venv/bin/python -m pytest -q && .venv/bin/ruff check src tests configs && .venv/bin/ruff format --check src tests && .venv/bin/mypy src tests scripts && .venv/bin/python -m pytest tests/test_forbidden_words.py -q`
Expected: `622 passed`, ruff clean, mypy `Success`, forbidden words 0.
...
Expected: `6 passed, 7 skipped` (every gpu-marked item reports `needs CUDA`), no collection error. The Triton tests themselves run on the pod: `pytest -m gpu` from the pod's clone before `pod.py run` (the amendment of Task 8 records this).
...
and add to the validation (before `if bad:`), with `from kvdlra.kernel.prompts import PROMPT_STARTS, PROMPT_TOKENS` at the top (torch-free):
...
Expected: all passed; `test_kernel_check_axis.py` under 1.5 s.

- [ ] **Step 5: Full suite + lint, then commit**

Run: `.venv/bin/python -m pytest -q && .venv/bin/ruff check src tests scripts && .venv/bin/ruff format --check src tests scripts && .venv/bin/mypy src tests scripts && .venv/bin/python -m pytest tests/test_forbidden_words.py -q`
Expected: `629 passed, 7 skipped`, ruff clean, mypy `Success`, forbidden words 0.
...
def test_the_cli_renders_an_existing_pod(tmp_path: Path) -> None:
    """`w19_sysfix_latency` has no kernel arm: the table renders and the verdict is REFUSED
    with the reason, never a traceback and never `--`."""
    import subprocess
    import sys
    ...
    out = tmp_path / "t.md"
    r = subprocess.run([sys.executable, str(tables.REPO_ROOT / "scripts/tables.py"), "latency",
                        "--pods", str(d), "--out", str(out)], capture_output=True, text=True)  # fmt: skip
...
Expected: all passed (`test_latency_table.py` under 1.5 s; the CLI test spawns one subprocess); `make -n` prints the `scripts/tables.py latency ...` line.

- [ ] **Step 6: Full suite + `make tables` diff-clean + lint, then commit**

Run: `.venv/bin/python -m pytest -q && make tables && .venv/bin/ruff check src tests scripts && .venv/bin/ruff format --check src tests scripts && .venv/bin/mypy src tests scripts`
Expected: `635 passed, 7 skipped`; `tables: diff-clean vs paper-v1`; ruff clean; mypy `Success`.
...
def _run(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run([sys.executable, "scripts/pod.py", *args], capture_output=True, text=True, cwd=REPO_ROOT)
...
def _harvested(tmp_path: Path, lines: list[str]) -> Path:
    d = tmp_path / POD
    r = _run("run", "--pod", POD, "--out", str(d), "--dry-run")
    assert r.returncode == 0, r.stderr
    m = json.loads((d / "manifest.json").read_text())
    m["dry_run"] = False
    (d / "manifest.json").write_text(json.dumps(m))
    (d / f"{POD}-1.log").write_text("\n".join(lines) + "\n")
    r = _run("harvest", "--pod", POD, "--log", str(d / f"{POD}-1.log"), "--out", str(d))
    assert r.returncode == 0, r.stdout + r.stderr
    return d


def test_a_complete_kernel_smoke_harvest_passes_check_and_renders(tmp_path: Path) -> None:
    d = _harvested(tmp_path, _lines())
    m = json.loads((d / "manifest.json").read_text())
    assert m["records"] == {"trials.jsonl": 0, "kernel_check.jsonl": 16, "latency.jsonl": 9}
    r = _run("check", str(d))
    assert r.returncode == 0, r.stdout + r.stderr
    out = tmp_path / "kernel_smoke.md"
    t = subprocess.run([sys.executable, "scripts/tables.py", "latency", "--pods", str(d), "--out", str(out)],
                       capture_output=True, text=True, cwd=REPO_ROOT)  # fmt: skip
...
def test_a_short_check_or_grid_fails_check_by_name(tmp_path: Path) -> None:
    d = _harvested(tmp_path, _lines(n_prompts=15))
    r = _run("check", str(d))
    assert r.returncode == 1
    assert "CHECK FAIL kernel_check: isvd_r64_h256_seed_kernel ctx=4096 has 15 of 16 prompt records" in r.stdout
    e = _harvested(tmp_path / "b", [x for x in _lines() if "ctx65536] isvd_r64" not in x])
    r = _run("check", str(e))
    assert r.returncode == 1
    assert "CHECK FAIL latency: isvd_r64_h256_seed_kernel ctx=65536 batch=1 has no decode record" in r.stdout
...
def test_launch_is_refused_from_a_dirty_or_unpushed_tree_and_needs_the_prereg() -> None:
    ...
    prereg as a strict ancestor is what the orchestrator's launch (D-011) must show."""
    r = _run("launch", "--pod", POD, "--offer", "12345678", "--dry-run")
...
doc: >-
  ...
  for the nine latency cells + ~0.4 h for the check = 2.2 h; gpu_budget_h is the 2.3x bar
...
Expected: all passed; `test_kernel_smoke_pod.py` under 2.5 s (three `pod.py` subprocesses per harvest test).
...
- **The axis.** `generator: kernel_check` (`kvdlra.eval.kernel_check`, commit `<L4.6 sha>`),
  ...
  attention function also computes reconstruct-then-attend (`_decode_peek` + sdpa) on the
  same live K/V and query for **every layer** and records `max|Δ|` per layer; the row keeps
...
- The check adds 16 prompts × 2 caches × (a 4096-token chunked prefill of the r64
  configuration ≈ 0.6 min at the measured 16K rate scaled by 1/4, plus 32 decode steps ≤ 0.1
  s each) ≈ **22 min ≈ 0.4 h** of compute. Point estimate **1.8 + 0.4 = 2.2 h**; the bar is
  **`gpu_budget_h: 5.0`** — 2.3× the point, the sibling pods' 2× discipline kept (§9's 4.0
  would be 1.8×). Cell list A's dollar figure moves to $1.0–1.6 expected and $2.3–3.7 at the
  bar at §9's $0.45–0.74/h.
...
Run: `.venv/bin/python -m pytest -q && .venv/bin/ruff check src tests scripts && .venv/bin/mypy src tests scripts && .venv/bin/python -m pytest tests/test_forbidden_words.py -q && make tables`
Expected: `639 passed, 7 skipped`; clean; `tables: diff-clean vs paper-v1`.
...
The pod (ADR §4: "one Gate-1-style row on Llama 16K, n = 24"): a single-arm pod paired against the Gate-1 Stage-1 Llama pod's `full` and `isvd_r64_h256_seed` rows by `prompt_sha256` (retrieval) and `window_idx` (perplexity) — the same task files, seed and design produce byte-identical prompts and windows, the pairing the pre-flight re-run already used across two pods (D-011 addendum 10, reading (iii)); the prereg refuses any key that does not pair. Rejected: a three-arm pod repeating `full` and `isvd_r64_h256_seed` (≈ 34 h bar, ≈ $25) for readings Stage 1 already buys.
...
fragments), one `ppl=` line, ≈ 32 `[diag]` rows per sample at `diag_every` 64 ≈ 4,100 rows
...
≈ 34 h for readings Stage 1 already buys; rejected.
...
rate, bar and credit. Amendments only, never edits.
...
Run: `.venv/bin/python -m pytest tests/test_rope_basis.py tests/test_golden_cache.py tests/test_bug_cache.py tests/test_kernel_path.py tests/test_config_parity.py tests/test_pod_manifest.py -q --durations=5 && .venv/bin/python -m pytest -q && .venv/bin/ruff check src tests scripts configs && .venv/bin/ruff format --check src tests scripts && .venv/bin/mypy src tests scripts && .venv/bin/python -m pytest tests/test_forbidden_words.py -q && make tables`
Expected: all passed (`test_rope_basis.py` under 1.0 s; the golden unchanged); `644 passed, 7 skipped`; clean; `tables: diff-clean vs paper-v1`. Then time the lane's additions: `.venv/bin/python -m pytest tests/test_batched_cache.py tests/test_kernel_prompts.py tests/test_kernel_reference.py tests/test_kernel_path.py tests/test_kernel_check_axis.py tests/test_latency_table.py tests/test_kernel_smoke_pod.py tests/test_rope_basis.py -q --durations=0 -p no:cacheprovider | tail -3` — Expected: the eight modules total ≤ 8 s on CPU (the dump test excluded, as in CI).

- [ ] **Step 6: Commit**

```bash
git add src/kvdlra/cache/bug_cache.py configs/arms/isvd_postrope_r128.yaml configs/pods/postrope_r128.yaml prereg/postrope_r128.md tests/test_rope_basis.py tests/test_config_parity.py tests/test_pod_manifest.py && git commit -o -m "L4.9: rope_basis knob (default pre, bit-identical; post = track post-RoPE keys, four identity sites) + the ADR 0001 (i) accuracy row: arm isvd_postrope_r128, pod postrope_r128 (paired against Stage 1 by prompt_sha256/window_idx, bar 24 h, not launched), prereg/postrope_r128.md" -- src/kvdlra/cache/bug_cache.py configs/arms/isvd_postrope_r128.yaml configs/pods/postrope_r128.yaml prereg/postrope_r128.md tests/test_rope_basis.py tests/test_config_parity.py tests/test_pod_manifest.py
```
