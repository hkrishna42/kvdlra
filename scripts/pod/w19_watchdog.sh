...
ROWS='^\[(niah|vt|persist|latency)[^]]*\] +[^ ].* (acc=|SKIP|bytes=|ms/tok=)|^ +[^ ].* \[T=[0-9]+\] (ppl=|OOM|error|mem alloc)|^\[pplw|^\[trial\]|^===(W19_|ALL_DONE|RUN_SHA|ENV_|QUANTO|HQQ|MODEL_)|^run_sha=|^device=|^torch=|^transformers=|NVIDIA'
extract(){  # local names: the caller's $lab must survive (commit message label)
  local l i m t
  while IFS=: read -r l i m t; do
    [ -z "$l" ] && continue
    sort -u "$H/${l}.raw" 2>/dev/null | grep -aE '^\[(niah|vt|persist|latency)[^]]*\] +[^ ].* (acc=|bytes=|ms/tok=)|^ +[^ ].* \[T=[0-9]+\] ppl=' > "results/w19-${l}-lines.txt"
