"""Week-10 Phase 1b: frozen-scoring (``score``) mode on the BUG cache.
...
def test_qlen_raise_still_fires_outside_score(tiny_model: LlamaForCausalLM) -> None:
    cache = _make_bug(tiny_model)
    ids = _prompt(40)
    with torch.no_grad():
...
def test_score_forward_is_non_mutating(tiny_model: LlamaForCausalLM) -> None:
    cache = _make_bug(tiny_model)
    ids = _prompt(60)
