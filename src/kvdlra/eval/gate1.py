...
an absent member is not evidence for it.
...
*before* the rules above and all returning ``UNDECIDED`` rather than raising -- a raise
would leave ``make gate1`` with no table in which to print the failure:
...
2. "With **no shared key** [``mcnemar_exact``] returns **None** ... a primary member at
   ``None`` refuses the verdict for its family exactly as an ``error`` row does."
...
from kvdlra.eval.stats import Key, holm, mcnemar_exact, paired_bootstrap, paired_t, tost
...
SweepKey = tuple[str, int, str]  # family, ctx, tracker
...
def _frozen_defects(path: Path, arm: str, freeze_after: int) -> list[dict[str, object]]:
    ...
    if not path.is_file():
        return []
    first: dict[tuple[object, object, object], int] = {}
    ...
    with path.open() as f:
        for line in f:
            ...
            sample = (row.get("task"), row.get("idx"), row.get("layer"))
    ...
    return [
        r
        for r in repairs
        if r["tokens_seen"] != first[(r.get("task"), r.get("idx"), r.get("layer"))]
    ]
...
def load(pod_dirs: list[Path]) -> Gate1Data:
    ...
    for d in pod_dirs:
        ...
        for w in (cast(PplwRecord, x) for x in _maybe(d / "pplw.jsonl")):
            sweep: SweepKey = (family, w["ctx"], _tracker(by_key, w["arm"], d / "pplw.jsonl"))
            if w["window_idx"] in bits[sweep]:
                raise ValueError(
                    f"{d}: {w['arm']} carries window_idx={w['window_idx']} twice"
    ...
    for f, c, t in sorted({(f, c, t) for f, c, t, k in data.hits if k == REFERENCE}):
        for b in (*PRIMARY_CONTROLS, *SECONDARY_CONTROLS):
            if (f, c, t, b) not in data.hits:
                continue
            m = mcnemar_exact(data.hits[(f, c, t, REFERENCE)], data.hits[(f, c, t, b)])
            draft.append(
                Contrast(
...
def ppl_contrasts(data: Gate1Data) -> list[PplContrast]:
    ...
    Windows are paired by ``window_idx`` and a pairing that is not exact is refused, as
    ``ppl_stats`` refuses it: an unpaired comparison of pooled numbers hides the effect
    it is measuring. Holm runs over the PRIMARY members only (section 6's 4-member
    family, per context length); the secondary contrasts are "uncorrected and
    descriptive, except the ``fd`` TOSTs the C branch names".
    """
    draft: list[PplContrast] = []
    for f, c, _ in sorted(k for k in data.bits if k[2] == REFERENCE):
        for b in TRACKERS:
            if b == REFERENCE or (f, c, b) not in data.bits:
                continue
            a_w, b_w = data.bits[(f, c, REFERENCE)], data.bits[(f, c, b)]
            if set(a_w) != set(b_w):
                raise ValueError(
                    f"ppl: {f} ctx={c} {b} scored windows {sorted(set(a_w) ^ set(b_w))}"
                    f" that {REFERENCE} did not (or the reverse) -- the pairing is broken"
                )
            ...
            draft.append(
                PplContrast(
                    family=f, ctx=c, a=REFERENCE, b=b, primary=b in PRIMARY_CONTROLS,
                    n_windows=len(d), d_bits=mean_d, lo=lo, hi=hi, p=paired_t(d),
                    p_holm=None, p_tost=max(p_lo, p_hi), equivalent=equivalent,
                )
            )  # fmt: skip
    eligible = {i: x.ctx for i, x in enumerate(draft) if x.primary}
    adjusted = _holm_by_group(draft, cast(dict[int, object], eligible))
    return [replace(x, p_holm=adjusted[i]) for i, x in enumerate(draft)]
...
def _beats(
    ...
    ppl: dict[tuple[str, str], PplContrast],
) -> str | None:
    ...
    # neither branch (prereg section 4 rule 1).
    if t and t.p_holm is not None and t.p_holm < ALPHA and t.d_bits < 0 and not t.equivalent:
        return (
...
def gate1_verdict(retrieval: list[Contrast], ppl: list[PplContrast], data: Gate1Data) -> Verdict:
    ...
    p_idx = {(c.family, c.b): c for c in ppl if c.ctx == VERDICT_CTX}
    ...
    for c in retrieval:
        if c.ctx == VERDICT_CTX and c.primary and c.p is None:
            refused[c.family].append(f"{c.family}/{c.task}: {c.a} vs {c.b} share no (seed, trial)")
    ...
    for family in families:
        if family in refused:
            continue
        routes = {b: _beats(family, b, r_idx, p_idx) for b in PRIMARY_CONTROLS}
        if all(routes.values()):
            separated.append(family)
            members += [r for r in routes.values() if r]
    ...
    for family in families:
        ...
        for b in C_CONTROLS:
            ...
            t = p_idx.get((family, b))
            if t is None:
                blockers.append(f"{family}: no perplexity TOST for {REFERENCE} vs {b}")
            elif not t.equivalent:
                blockers.append(
                    f"{family}: {REFERENCE} vs {b} TOST at +/-{PPL_DELTA_BITS} does not pass"
                    f" (d={t.d_bits:+.4f} bits, p={t.p_tost:.2g})"
                )
    ...
    if is_ab:
        branch = "A/B"
        head = f"{len(separated)} families separated at 16K ({', '.join(separated)}): " + "; ".join(
            members
        )
    elif is_c:
        branch, members = "C", ["no member separates isvd from fd or frozen"]
        head = (
        ...
        branch = "UNDECIDED"
        members = refusals + blockers
        parts = []
        if refusals:
            parts.append(f"verdict refused for {', '.join(sorted(refused))}: " + _first(refusals))
        parts.append(f"one family separated ({separated[0]})" if separated else "nothing selected")
        parts.append("C blocked by: " + _first(blockers) if blockers else "C not readable")
        head = "; ".join(parts)
    return Verdict(branch, "; ".join([head, *notes]), separated, members)


def _first(lines: list[str], keep: int = 6) -> str:
    rest = len(lines) - keep
    return ", ".join(lines[:keep]) + (f" (+{rest} more)" if rest > 0 else "")
