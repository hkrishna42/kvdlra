...
Started in Task 6; Tasks 7 and 9 append to it.
...
| `src/kvdlra/integrators/oja.py` | 310 | tests | reachability §3b. The Week-20 tracker swap runs the **torch** `oja_step` inside `tracker/isvd.py` (audit "Surprise check"; `cache/bug_cache.py:159`), not this numpy original. |
...
| `src/kvdlra/quant/qjl.py` | 109 | pod-sh+tests → none | reachability §2 L140. audit: "exported by `quant/__init__.py:7` and imported by nothing but `tests/test_qjl.py`". The re-export is the whole pod-sh edge (`bug_cache.py:160`/`lowrank_press.py` import `kvdlra.quant`, which executes `__init__`); removing `QJL` from `quant/__init__.py` severs it. `grep -rn QJL src scripts tests` finds no other user. |
...
|---|---|---|---|
| `src/kvdlra/quant/product_quant.py` (+ `tests/test_product_quant.py`) | 300 + 163 | `src/kvdlra/cache/bug_cache.py:160` imports `ProductQuantizer` **at module level**, and `bug_cache.py` survives L0. `tests/test_bug_cache_seed_regression.py:31` and `tests/test_bug_cache_week8.py:30` also construct it. The audit pairs this deletion with the CodeBUG path it feeds. | Task 7 (removes the CodeBUG path, then this) |
| `src/kvdlra/cache/morph_cache.py` (+ `tests/test_morph_cache.py`) | 449 + 260 | The paper-row rule is satisfied — `git grep -n -i morph paper/main.tex` prints **nothing at all** (no table row, no prose) — but the module is not free-standing: `src/kvdlra/cache/bug_cache.py:158` imports `_aggregated_attention_row, _window_attention_rows` from it **at module level** (they implement the `attn` retention mode), `src/kvdlra/cache/__init__.py:4` re-exports `MorphKVCache`/`MorphKVLayer`, `src/kvdlra/accounting.py:282` (`morph_footprint`) bills a MorphKV layer, and three surviving tests build one (`test_accounting.py:190`, `test_w10_ingest.py:129`, `test_w10_score_mode.py:85`). Bucket is `pod-sh+tests` (reachability §2 L124). `scripts/w10_frontier.py:build_arms` and its `--methods morph` choice were therefore left alone. | Task 7 (deletes the `attn` retention mode) — then the import is gone and this can follow |
