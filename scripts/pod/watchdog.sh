...
# record), and on its own ===ALL_DONE_<mode>: destroy it, dedupe the rows into
# <label>.log and turn them into records with `scripts/pod.py harvest`. It commits
# nothing and pushes nothing -- the orchestrator commits the harvest.
...
ROWS='^\[(niah|vt|persist|latency)[^]]*\] +[^ ].* (acc=|SKIP|bytes=|ms/tok=)|^ +[^ ].* \[T=[0-9]+\] (ppl=|OOM|error|mem alloc)|^\[pplw|^\[diag|^\[trial\]|^===(ALL_DONE|POD_|RUN_SHA|ENV_|QUANTO|HQQ|MODEL_)|^run_sha=|^device=|^torch=|^transformers=|NVIDIA'
for iter in $(seq 1 "$BUDGET_ITERS"); do
  while IFS=: read -r lab id mode tag; do
    ...
    if echo "$L" | grep -qaE "===ALL_DONE_${mode}"; then
