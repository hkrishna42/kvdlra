...
tables are $n{=}12$ and the marquee is $n{=}16$, all from a single September run
in which pods were cloned by \emph{commit SHA} (not branch), stamped their
git-SHA and GPU/torch/CUDA environment into the harvested logs, and emitted
per-trial hit records. The marquee retrieval and perplexity are same-pod
(\S\ref{sec:marquee}). The residual provenance gap is only the earlier
cross-week 1B/W-15 material and a formal compute-disclosure appendix, which is
v2 hygiene.
