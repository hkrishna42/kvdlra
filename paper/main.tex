...
2-bit baseline at matched bytes it wins multi-value retrieval (pooled paired
records $21$ vs $0$) but trails or ties on fluency; 4-bit quantization at twice the
...
multi-value retrieval on Llama and Mistral---a depth no fixed-bit quantizer
reaches, and at which every eviction$\times$quantization composite we ran
collapses on the official anchor (mean $\le0.19$)---at a fluency cost, and not on
Qwen. We map the limits (an $r/n\approx0.25$ retrieval wall; a rank-siphoning
...
(reconstruct-then-attend; decode residency $\approx1.06\times$ until a fused
kernel). Its measured payoff is a $\approx7\times$ smaller persisted cache
...
gist tracker---is the test of whether the tracker is load-bearing, and is reported
in \S\ref{sec:limits}.
...
\emph{neither} scalar quantization \emph{nor} eviction$\times$quantization---with the
honest caveat that the cell itself has no official-anchor number (\S\ref{sec:limits}),
on a suite where the flagship's own multi-value edge does not transfer either.
...
``off.'' column at or below plain eviction, and the $0.070\times$ composite reaches
$0.33$. Mistral rows are our generator only.}
\label{tab:composite}
\small
\begin{tabular}{lccc}
\toprule
...
\midrule
\texttt{bugSseed-r64-h256-q4} (cell) & $0.048$/$0.034\times$ & $1.00/1.00/1.00/0.50$ \;/\; $1.00/1.00/1.00/0.83$ & --- \\
...
composed sub-cliff cell (\S\ref{sec:subcliff}) has no official-anchor number of its
own; its retrieval is our-generator-only, though the eviction$\times$quantization
composites it is measured against (Table~\ref{tab:composite}) were run on the official
anchor and collapse there, as plain eviction does.
...
eviction$\times$quantization composite reaches with retrieval at matched bytes is the
composed gist-plus-4-bit-coordinates cell at $0.048\times$ on Llama and Mistral
(measured against both, \S\ref{sec:subcliff}; family-dependent, at a fluency cost). Low-rank tracking is an axis orthogonal to eviction and
